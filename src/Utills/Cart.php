...
{
    /** book category id */
    CONST CATEGORY_CHILD_BOOK = "child";
     ...
     * Get all Items
     *
     * @return \AppBundle\Utils\CartItem[] An array of Items,  if no Items then an empty array
     */
     ...
     *
     * @param \AppBundle\Utils\CartItem $item
     */
    ...
    public function getCategoriesByCount($category = null)
    {
        ...
        foreach ($this->getItems() as $item) {
            ...
            $categories[$index] = isset($categories[$index]) ? $categories[$index] + $item->getQuantity() : $item->getQuantity();
        }
        ...
        if (! is_null($category)) {
            ...
            if (!empty($categories) && ! is_null($categories)) {
                $result = isset($categories[0]) ? $categories[0] : null;
            } else {
                $result = 0;
            }
        }
    ...
    public function getPriceByCategory($category = null)
    {
        ...
        foreach ($this->getItems() as $item) {
            ...
            $priceByCategory[$cat] = isset($priceByCategory[$cat]) ? $priceByCategory[$cat] + $itemTotal : $itemTotal;
        }
